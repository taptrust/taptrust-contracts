...
/**
 * Verifier Registry Tasks
 *
 * These tasks can be used to interact with the contract state.
 * Each task maps with the methods found on the IVerifierRegistry.sol
...
task("registryAddVerifier", "Add a Verifier to the contract")
  ...
  .setAction(async (taskArgs, hre) => {
    const registryFactory: ContractFactory =
      await hre.ethers.getContractFactory("Verifier")
...
/**
 * Verified Credential Registry Tasks
 *
 * These tasks can be used to interact with the contract state.
 * Each task maps with the methods found on the IVerifiedCredentialRegistry.sol
 * interface. Please see that file for up-to-date comments and implementation
 * notes.
 */
...
task(
  ...
  .setAction(async (taskArgs, hre) => {
    const registryFactory: ContractFactory =
      await hre.ethers.getContractFactory("VerifiedCredentialRegistry")
    const registry = registryFactory.attach(taskArgs.registry)
